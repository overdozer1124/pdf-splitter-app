...
- 先頭ページのサムネイルプレビュー表示
- **📑 ファイル名からの自動しおり（目次）作成**:
  - 各PDFの先頭ページに、元ファイル名のしおり（アウトライン）を自動埋め込み
  - 日本語文字化け防止（UTF-16BEエンコード）対応
- **📦 柔軟なグループ指定結合**:
  - **全ファイルを1つに結合**: 単一PDF出力
  - **指定ファイル数ごと**: 指定件数（例: 3ファイル）ごとに分割結合
  - **指定ページ数ごと**: 合計指定ページ（例: 20ページ）以内ごとに分割結合
  - 複数ファイル生成時は自動でZIP一括ダウンロード
- 指定したファイル名で高速出力
